...
require('lazy').setup({ -- NOTE: Plugins can be added with a link (or for a github repo: 'owner/repo' link).
  ...
  { -- Fuzzy Finder (files, lsp, etc)
    ...
    config = function()
      ...
      end, { desc = '[S]earch Recent Files in Current Dir ("." for repeat)' })
    ...
    config = function()
      ...
      vim.api.nvim_create_autocmd('LspAttach', {
        ...
        callback = function(event)
          ...
          -- 从 Neovim v0.11 版本开始，为了提供更好的开箱即用的 LSP 体验，引入了一系列默认的 LSP 快捷键: gr*
          -- grn : 调用 LSP 进行重命名
          -- gra : 显示可用的代码操作 (Code Actions)
          -- grr : 列出光标下符号的所有引用 (References)
          -- gri : 列出光标下符号的所有实现 (Implementations)
          -- grd : 列出光标下符号的所有定义 (Define)
          -- grt : 列出光标下符号的所有类型定义 (Type Definition)
          -- grD : 列出光标下符号的所有声明 (Declare)

          -- remapping by useful short keys
          ...
          map('<leader>sO', require('telescope.builtin').lsp_document_symbols, 'Open Document Symbols')
      ...
      sources = {
        ...
        per_filetype = {
          AvanteInput = {
            ...
            'avante_mentions',
            'avante_shortcuts',
            -- 'avante_files', -- Not available in current avante commit.
          },
          AvantePromptInput = {
            inherit_defaults = false,
